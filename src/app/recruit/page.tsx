...
export async function generateMetadata({ searchParams }: Props): Promise<Metadata> {
  ...
  const title = area ? `${area}の求人情報` : "東海エリアの求人情報";
  const description = area
    ? `${area}のフィリピンパブ・スナック・ガールズバー等の求人情報。給与・待遇・シフトを確認して気軽に応募できます。`
    : "東海エリア（浜松・静岡・名古屋・岐阜）のフィリピンパブ・スナック・ガールズバー等の求人情報一覧。未経験歓迎の求人も多数。";
  return {
    title,
    description,
    alternates: {
      canonical: `${process.env.NEXT_PUBLIC_SITE_URL ?? ""}/recruit${area ? `?area=${encodeURIComponent(area)}` : ""}`,
    },
  };
}
    ...
    <div className="max-w-4xl mx-auto px-4 py-10">
      ...
      <div className="mb-8">
        <h1 className="text-2xl font-black text-white mb-2">
          💼 {area ? `${area}の求人情報` : "東海エリアの求人情報"}
        </h1>
        <p className="text-gray-400 text-sm">
          {storeList.length > 0
            ? `${storeList.length}件の求人を掲載中`
            : "現在掲載中の求人はありません"}
        </p>
      </div>
      ...
      <div className="flex flex-wrap gap-2 mb-8">
        <Link
          ...
          className={`text-xs px-3 py-1.5 rounded-full font-bold border transition-colors ${
            !area
              ...
              : "border-dark-border text-gray-400 hover:border-pink-400/60 hover:text-white"
          }`}
        >
          すべて
        </Link>
        {ALL_AREAS.map((a) => (
          <Link
            key={a}
            href={`/recruit?area=${encodeURIComponent(a)}`}
            className={`text-xs px-3 py-1.5 rounded-full font-bold border transition-colors ${
              area === a
                ? "bg-pink-500 border-pink-500 text-white"
                : "border-dark-border text-gray-400 hover:border-pink-400/60 hover:text-white"
